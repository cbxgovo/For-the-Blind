...
def work(request):
    ...
    # client.on_connect = on_connect
    client.connect(broker, port, 60)  # 连接
    client.publish(topic, payload="1", qos=0)  # 发布消息
    client.disconnect(broker, port)
    # client.loop_forever()  # 保持连接状态
...
def contact(request):
    ...
    def on_message(client, userdata, msg):
        ...
        global namegps
        global namegps_split_w
        global namegps_split_j
        namegps = msg.payload.decode()
        print("name1连接中断前的保留测试:" + namegps)
        client.disconnect(broker, port)
        print("name1连接中断后的保留测试:" + namegps)
...
def button2_fx_view(request):
    ...
    def on_message(client, userdata, msg):
        ...
        # 只将超声波的图像传入数据库
        models.Iot.objects.create(dis=name1_split_ul)
